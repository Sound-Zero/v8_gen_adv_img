...
class Results(SimpleClass):
    ...
    属性:
        orig_img (numpy.ndarray): 原始图像，存储为 numpy 数组。
        orig_shape (Tuple[int, int]): 原始图像形状，格式为 (高度, 宽度)。
        boxes (Boxes | None): 包含检测边界框的对象。
        masks (Masks | None): 包含检测掩码的对象。
        probs (Probs | None): 包含分类任务类别概率的对象。
        keypoints (Keypoints | None): 包含每个对象检测到的关键点的对象。
        obb (OBB | None): 包含定向边界框的对象。
        speed (Dict[str, float | None]): 包含预处理、推理和后处理速度（ms/图像）的字典。
        names (Dict[int, str]): 将类别 ID 映射到类别名称的字典。
        path (str): 图像文件路径。
        _keys (Tuple[str, ...]): 用于内部使用的属性名称元组。

    方法:
        update: 使用新的检测结果更新对象属性。
        cpu: 返回所有张量都在 CPU 内存中的 Results 对象副本。
        numpy: 返回所有张量都转换为 numpy 数组的 Results 对象副本。
        cuda: 返回所有张量都在 GPU 内存中的 Results 对象副本。
        to: 返回所有张量都在指定设备和数据类型的 Results 对象副本。
        new: 返回具有相同图像、路径和名称的新 Results 对象。
        plot: 在输入图像上绘制检测结果，返回带注释的图像。
        show: 在屏幕上显示带注释的结果。
        save: 将带注释的结果保存到文件。
        verbose: 返回每个任务的日志字符串，详细说明检测和分类结果。
    ...
    def __init__(
        ...
        """
        初始化 Results 类，用于存储和处理推理结果。

        参数:
            orig_img (numpy.ndarray): 原始图像，存储为 numpy 数组。
            path (str): 图像文件路径。
            names (Dict): 类别名称字典。
            boxes (torch.Tensor | None): 每个检测的边界框坐标，2D 张量。
            masks (torch.Tensor | None): 检测掩码，3D 张量，每个掩码是一个二值图像。
            probs (torch.Tensor | None): 分类任务中每个类别的概率，1D 张量。
            keypoints (torch.Tensor | None): 每个检测的关键点坐标，2D 张量。
            obb (torch.Tensor | None): 每个检测的定向边界框坐标，2D 张量。
...
class Probs(BaseTensor):
    ...
    该类继承自 BaseTensor，提供了访问和操作分类概率的方法，包括 top-1 和 top-5 预测。

    属性:
        data (torch.Tensor | numpy.ndarray): 包含分类概率的原始张量或数组。
        orig_shape (tuple | None): 原始图像形状 (高度, 宽度)。该类中未使用。
        top1 (int): 最高概率类别的索引。
        top5 (List[int]): 前 5 个概率类别的索引。
        top1conf (torch.Tensor | numpy.ndarray): 最高概率类别的置信度分数。
        top5conf (torch.Tensor | numpy.ndarray): 前 5 个概率类别的置信度分数。

    方法:
        cpu(): 返回概率张量的 CPU 内存副本。
        numpy(): 返回概率张量的 numpy 数组副本。
        cuda(): 返回概率张量的 GPU 内存副本。
        to(*args, **kwargs): 返回具有指定设备和数据类型的概率张量副本。
    ...
    def __init__(self, probs, orig_shape=None) -> None:
        """
        初始化 Probs 类，用于存储分类概率。

        该类存储和管理分类概率，提供对 top 预测及其置信度的便捷访问。

        参数:
            probs (torch.Tensor | np.ndarray): 分类概率的 1D 张量或数组。
            orig_shape (tuple | None): 原始图像形状 (高度, 宽度)。该类中未使用，但为了与其他结果类保持一致而保留。

        属性:
            data (torch.Tensor | np.ndarray): 包含分类概率的原始张量或数组。
            top1 (int): 最高概率类别的索引。
            top5 (List[int]): 前 5 个概率类别的索引。
            top1conf (torch.Tensor | np.ndarray): 最高概率类别的置信度。
            top5conf (torch.Tensor | np.ndarray): 前 5 个概率类别的置信度。

...
def non_max_suppression(
    ...
    参数:
        prediction (torch.Tensor): 形状为 (batch_size, num_classes + 4 + num_masks, num_boxes) 的张量，
            包含预测的边界框、类别和掩码。张量应为模型（如 YOLO）输出的格式。
        conf_thres (float): 置信度阈值，低于此值的边界框将被过滤掉。
            有效值在 0.0 到 1.0 之间。
        iou_thres (float): IoU 阈值，低于此值的边界框在 NMS 过程中将被过滤掉。
            有效值在 0.0 到 1.0 之间。
        classes (List[int]): 要考虑的类别索引列表。如果为 None，则考虑所有类别。
        agnostic (bool): 如果为 True，模型将忽略类别数量，所有类别将被视为一个类别。
        multi_label (bool): 如果为 True，每个框可能有多个标签。
        labels (List[List[Union[int, float, torch.Tensor]]]): 列表的列表，其中每个内部列表包含给定图像的先验标签。
            列表应为数据加载器输出的格式，每个标签为 (class_index, x1, y1, x2, y2) 的元组。
        max_det (int): NMS 后保留的最大边界框数量。
        nc (int, 可选): 模型输出的类别数量。此后的索引将被视为掩码。
        max_time_img (float): 处理单张图像的最大时间（秒）。
        max_nms (int): 输入 torchvision.ops.nms() 的最大边界框数量。
        max_wh (int): 边界框的最大宽度和高度（像素）。
        in_place (bool): 如果为 True，输入预测张量将被原地修改。
        rotated (bool): 如果为 True，表示传递的是旋转边界框（OBB）用于 NMS。
        end2end (bool): 如果模型不需要 NMS。

    返回:
        (List[torch.Tensor]): 长度为 batch_size 的列表，其中每个元素是一个形状为 (num_boxes, 6 + num_masks) 的张量，
            包含保留的边界框，列分别为 (x1, y1, x2, y2, confidence, class, mask1, mask2, ...)。
    """
